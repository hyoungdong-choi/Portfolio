...
### 개요
- **목적**: 사내에서 이미 구축된 MSA 아키텍처를 관리하고, Rancher를 통해 클러스터 상태와 리소스를 관리  
- **주요 기술**:  
...
- **성과**:  
  ...
  - TiDB로 인한 etcd동기화 이슈 및 기존 HDD -> SSD로 전환하여 에러 및 경고 해결
- **어려움 & 해결**:  
  - 기존 MSA 구조가 복잡해 리소스 배포 시 연결 부분 오류 -> 전체 워크플로우 파악 후 로드맵을 통해 정리
  - TiDB에서 Recommended된 서버 스펙으로도 에러가 발생 -> SSD 및 TiDB설정 값으로 인한 이슈 처리
---
